...
with col1:
    replicate_status = "✅" if current_keys.get('REPLICATE_API_TOKEN', '').startswith('r8_') else "❌"
...
with col3:
    elevenlabs_status = "✅" if current_keys.get('ELEVENLABS_API_KEY', '') and current_keys.get('ELEVENLABS_API_KEY') != 'your_elevenlabs_key_here' else "⚠️"
...
with col2:
    if st.button("🟢 تشغيل الخدمات"):
        ...
        os.system("cd /workspaces/sa && /workspaces/sa/.venv/bin/streamlit run src/sa/ui/app.py --server.port 8501 --server.address 0.0.0.0 --server.enableXsrfProtection false --server.enableCORS false > logs/streamlit.log 2>&1 &")
        os.system("cd /workspaces/sa && /workspaces/sa/.venv/bin/uvicorn sa.api:app --host 0.0.0.0 --port 8000 > logs/api.log 2>&1 &")
